...
**Estrutura**
```
src/app/            rotas: / · /cases · /cases/[slug] · /vagas · /aws · sitemap.ts · robots.ts
src/content/        conteúdo como dados tipados: types.ts · cases.ts (20) · capabilities.ts (6) · scenes.ts (4) · segments.ts · site.ts · jobs.ts (vagas) · partners.ts (AWS e novas parcerias)
src/components/
...
**Rotas e redirects:** `/aws.html` → `/aws`; `/portfolio.html` → `/cases`; `/index.html` → `/`.
...
- `/cases` — design V3 da equipe (Figma `125:317`; filtrado em `108:2446`, 24/09). "Nossos cases" + filtros à esquerda (Serviços empilhados, Segmento em linha; ativo em Azul Digital com texto preto; "Limpar filtros" no fim dos filtros, depois de Segmento, só com filtro ativo; sem Cloud e DevOps na AWS — `filterCapabilities` em capabilities.ts, a mesma lista da home e do rodapé) e 20 cards de 283px (surface-2, borda 2px line-strong, nome Syne 800 32px, resumo, até 3 indicadores e as capacidades em etiquetas). Ordem do design em `casesInOrder` (com imagem primeiro), usada também no anterior/próximo. No celular as telas do card do AVA ficam a 78% (`--art-fit` em `AnimatedCaseCard.module.css`, 25/09) — subiam até cobrir a descrição e a etiqueta; a animação da Duda não mudou.
...
- `/aws` — "Serviços AWS", layout da equipe no Figma (V4, frame 203:37, 25/09): trilha parcerias / aws e h1; três frentes (Migrar e modernizar · Construir na nuvem · Operar com DevOps) separadas por hairline, cada uma com o numeral grande em Azul Solução (70%) atrás do título e os serviços em cartões brancos (Syne 700 29px, texto 18px preto) com etiquetas mono brancas sobre Azul Solução; a frente do meio é espelhada (título à direita no desktop). Cartões em duas colunas; com número ímpar de serviços o último ocupa a linha. Fecha com a faixa Azul Solução com o selo oficial AWS Partner Select Tier Services (`public/parcerias/aws-partner-select-tier.png`, PNG da equipe 313×270) e "Fale conosco" → contato com assunto "Soluções AWS". Textos e etiquetas são os da /aws.html antiga (partners.ts). No Figma o 3º cartão de "Construir na nuvem" veio como "Microsserviços" com o texto da IA generativa — corrigido lá e no código.
- `/vagas` — "Nossas vagas", mockup da equipe (24/09); é para onde vai o "Acessar vagas" do Trabalhe conosco (e o submenu "Nossas vagas" da Nav) (antes ia direto ao Pandapé). Mesmo esqueleto do /cases: à esquerda "Carreiras", busca (título e área, sem diferenciar acento) e filtro de área de escolha única ("Ver todos" ativo em Azul Digital); à direita cards de vaga (surface-2, borda 2px line-strong): título Syne 800 28px, modalidade e contrato, salário em Azul Solução + "+benefícios", "Inscrições até dd/mm" e "Ver oportunidade ↗". **As 8 vagas são as do mockup, de exemplo** (`src/content/jobs.ts`); o formato é plano, uma coluna por campo, porque o plano é ligar a lista a uma planilha que a equipe preenche e a IA transforma em vagas. "Ver oportunidade" aponta para o Pandapé até existir link por vaga.

**Nav — mockup da equipe (23/09).** No topo da página, largura do container: símbolo à esquerda, links no centro (cinza), "Fale conosco" em contorno à direita. Ao rolar (compacta acima de 40px, volta abaixo de 8px — histerese), o símbolo e o botão deslizam para junto dos links (animação de layout do Motion, mola) e o conjunto vira uma barra preta compacta flutuando a 8px do topo, centralizada (~600 × 58, cantos retos), links em branco e botão sólido branco com texto Neutra 200. O fundo preto entra com 200ms de atraso, depois que os itens se juntaram. O `header` segue `sticky` com 4rem no fluxo, mas é `pointer-events-none` — só a barra recebe cliques. Abaixo de `md` não compacta: é uma faixa transparente com o símbolo e o botão do menu (menu lateral, sem números); desde 25/09 ela não ganha mais fundo ao rolar — o botão tem fundo preto próprio para seguir visível sobre o laranja e o branco. **Z-index:** `z-nav`/`z-overlay`/`z-dialog` não geram CSS no Tailwind v4 (não existe namespace de z-index no `@theme`); use `z-(--z-nav)` etc. Antes da correção o cabeçalho ficava por baixo das seções ao rolar. **Submenus (25/09):** Cases, Parcerias e Trabalhe conosco têm um caret e, no hover ou com foco dentro (`group-hover`/`group-focus-within`), abrem uma caixa preta com borda embaixo do item: "Ver todos" → `/cases`, "Serviços AWS" → `/aws`, "Nossas vagas" → `/vagas`. O item continua indo para a âncora da seção na home (o pedido dizia "âncora dos cases" para Parcerias — tratado como engano; vai para `#parcerias`). A caixa tem `w-max` (sem isso o item estreito a espremia e a seta sumia) e um `pt-3` de ponte para o mouse. No menu lateral o sublink aparece embaixo do item, menor e cinza.

**Hero — nova versão da Duda (commit f1b4ef0, 25/09).** Abertura (só no primeiro carregamento da visita — voltando à home por link interno, ou entrando por outra página, o Hero já nasce pronto; `lib/site-entry.ts` + `SiteEntry` no layout, 25/09) com o símbolo 3D pequeno no centro e contador de 0 a 100%; depois ele cresce e desliza para a direita, e entram "SENAI / Soluções Digitais" e "Produtos digitais que transformam [a indústria | a educação | negócios]" (palavra digitada). Fundo **todo preto** com a malha de 96px em branco a 7% — em CSS (`Hero.module.css`), a mesma da cena de cases; os SVGs esticados da Duda (`grid.svg`, com brilho laranja, e `grid-loading.svg`) saíram em 25/09 porque as células só tinham 96px a 1440 de largura e não emendavam. O símbolo fica numa camada `fixed`. **Acabamento do símbolo (25/09, referência studiors.be):** preto brilhante — `MeshPhysicalMaterial` escuro com verniz (clearcoat), reflexos de um ambiente de estúdio gerado em runtime (`RoomEnvironment` + PMREM, sem HDR para baixar), chanfro redondo de 8 segmentos que vira um fio de luz nas arestas, luzes de recorte brancas por trás e um granulado fino (textura em canvas, semente fixa) na rugosidade. **Toque de laranja por baixo:** o chão do estúdio refletido é uma placa laranja SENAI acesa (intensidade ~2) e duas luzes vindas de baixo reforçam — faces e arestas viradas para baixo ficam quentes, o resto segue preto. **Complemento frio:** um painel Azul Solução no ambiente (esquerda-alto, mais fraco que o laranja) e uma luz de recorte da mesma cor — aparece como um tom azul em algumas faces internas conforme a peça gira. O pôster (`public/hero/mark-poster.png`, 983px) é o primeiro quadro do próprio canvas, para a troca pôster → 3D não mudar de cara. Ajustes de 25/09 (tarde): (1) **sem piscada na abertura** — em `LogoMark3D.tsx` o pôster (`mark-poster-warm.png`) é uma camada que nunca desmonta e o canvas fica por cima, invisível, até desenhar o primeiro quadro; os dois trocam por opacidade em 700ms (antes o pôster desmontava e o canvas passava alguns quadros vazio); (2) o símbolo **acompanha o texto** (pedido de 25/09, sem desbotar): a camada dele saiu de `fixed` e fica dentro do quadro preso (`.stage`), então para e sai junto com o texto. O quadro corta só na horizontal (`overflow-x: clip`), e o Hero tem `z-index: 11` (acima dos cases, `z-10`): o que o símbolo passa da base fica por cima do topo dos cases, sem traço reto; (3) o texto fica parado enquanto o quadro está preso e sai com a seção (saiu o deslocamento que o fazia descer e subir); (4) **celular**: o símbolo vem antes do texto, no alto do quadro (330px), **alinhado à esquerda com o texto** (o desenho começa a 17,4% da caixa, medido no pôster; `--mark-left: calha + 32,6% da caixa`), e o texto começa logo abaixo da caixa dele.

**Passagem Hero → Cases (25/09).** O Hero não tem laranja; a virada acontece na passagem. A seção de cases tem fundo laranja e, no topo do quadro fixo, entre a malha e o conteúdo, uma cortina preta (`bg-dawn`, globals.css) de 85svh que desbota em arco: máscara elíptica centrada na base (o laranja sobe primeiro pelo centro) cruzada com um degradê vertical que zera a base, sem emenda com o laranja chapado. O laço de scroll da cena a desloca com o topo da seção (`translateY(min(0, top))`), então ela sobe como se estivesse presa à seção. O título "Nossos cases" nasce sobre o mesmo preto do Hero. A malha (96px, branco a 7%) fica **por cima** da cortina e é a mesma do Hero: a do Hero é ancorada na base do quadro, que é a borda da seção quando ela sai, e a dos cases começa no topo — as linhas atravessam a passagem sem corte, no mesmo passo. Substituiu a entrada "líquida" (`useLiquidEntrance`, clip-path em onda) do commit da Duda e o arco antigo (`bg-arc`/`bg-arc-tail`). "Ver o case" passou a 19px mono em negrito com sublinhado de 2px: texto grande para o WCAG (≥ 14pt negrito), em que o branco sobre o laranja (3,91:1) passa AA.
...
**Globo 3D — Serviços → Sobre nós → Trabalhe conosco (24/09).** `GlobeTrack` envolve as três seções e desenha um único globo de arame (`GlobeScene`, three.js: 36 meridianos + 17 paralelos, eixo inclinado como em `public/home/globo.svg`, linhas em blending aditivo — os polos brilham sozinhos) numa camada `fixed` atrás do conteúdo (`isolate` + `-z-10`; por isso o About e a faixa preta do Trabalhe conosco não têm fundo próprio). A posição sai de três marcadores: `GlobeAnchor` à direita de Serviços (sangrando 5% pela borda), `GlobeSlot` no Sobre nós e `GlobeEnd` no fim da faixa preta do Trabalhe conosco. Quatro marcadores: `GlobeAnchor` (Serviços), `GlobePartner` (Parcerias, embaixo à esquerda atrás do título, 38vw/547px sangrando 207px pela borda — Figma 204:569), `GlobeSlot` (Sobre) e `GlobeEnd`. Em Serviços o globo espera na tela enquanto a lista rola; quando Parcerias entra, desliza (encolhendo) até o `GlobePartner` e pousa quando a dobra prende, ficando preso com ela; quando o Sobre entra, sai de Parcerias subindo com ela (passa por trás da faixa azul) e chega ao slot quando o quadro do Sobre prende. Cada pouso é a posição do marcador com o quadro da seção preso (`data-globe-frame`). Quando o Sobre solta, espera na mesma altura e, em ~0,8 tela de scroll, desce para a direita crescendo (648 → 757px a 1440) até virar a cúpula centrada embaixo do carrossel, cortada pela faixa branca, com "VEM SER FIESC" (Syne 800, 56px, caixa alta) por cima — pouso quando o fim da faixa preta encosta na base da tela. A faixa branca cobre o globo porque tem fundo; o texto é cortado por `overflow-hidden` da área. A altura de pouso é a do slot com o quadro preso (`data-globe-frame`, mesmo `top` do About), então não há salto vertical. A camada é recortada pela caixa das três seções (`clip-path`) — não aparece nos cases nem no Entre em contato. O scroll também gira o globo; nas travessias posição e giro são amortecidos, mas preso a um marcador (Serviços, subida com o Sobre, cúpula) a altura segue o marcador sem atraso — amortecida, ela ficava para trás ao rolar de volta e o recorte do topo cortava o globo (bug de 25/09). Tamanho `--globe: min(45vw, 40,5rem)`. **Tamanho por escala (25/09, vídeo "Erros globo"):** a camada fixa tem o tamanho do maior marcador e encolhe com `transform: scale()`; o canvas mede sem a escala (`resize={{ offsetSize: true }}`) e não é redimensionado no scroll. Antes a largura mudava a cada quadro, o canvas acompanhava com atraso e, ao encolher, passava da camada — o `clip-path` o cortava num traço reto. Na primeira aparição a camada fica escondida até o canvas ter o tamanho certo (nasce 300 × 150). **Sem globo no celular** (feedback 207:744): abaixo de `lg` o slot do Sobre e a cúpula do Trabalhe conosco não aparecem; o "Vem ser FIESC" fica centralizado e quebra em duas linhas. Abaixo de `lg`, com `prefers-reduced-motion` ou sem WebGL, sem travessia: o slot do Sobre e a cúpula do Trabalhe conosco mostram o SVG parado.

**Parcerias (25/09).** Logo depois de Serviços, dentro do GlobeTrack e **sem fundo próprio** (o globo passa por ela). Layout da equipe no Figma (204:569): à esquerda "Parceria" (mono 16px), "SENAI / Soluções / Digitais / + AWS" (Syne 800 74px, o "+ AWS" em laranja `mark`) e o botão "Nossos serviços AWS" → `/aws` (era "Conheça nossa atuação com AWS"), distribuídos na altura da dobra; à direita a ilustração `AwsJourney.tsx` — três losangos empilhados (388×256, 109,6 entre eles): branco com o selo AWS Partner aplicado com a matriz do Figma, laranja SENAI 70% e Azul Conectado 70%, sombra 0 46 14 preta 30% nos dois de cima —, cada um ligado por uma linha de 49px (branca / laranja / Azul Robótico) ao título (Migrar e modernizar / Construir na nuvem / Operar com DevOps — as três frentes da /aws); embaixo dela o texto da APN (`partnersHome.intro`). Depois, faixa Azul Solução "Seja nosso parceiro" com "Contato" → `#contato` com assunto "Proposta de parceria" (`src/lib/contact-topic.ts`). A ilustração é construída pelo scroll **de cima para baixo**: no desktop a dobra prende (`sticky` + espaçador de 1 tela) e o progresso começa quando a base da ilustração chega a 95% da tela; em cada plano o contorno é traçado a partir do vértice esquerdo, guias verticais descem do plano de cima (e somem), a linha do título cresce e o título sobe, entram superfície e sombra, o traço recua; o selo por último. Coreografia em atributos `data-l/-a/-k` (cabeçalho do arquivo). Pronta, os planos flutuam 3px. Abaixo de `lg` não prende; com prefers-reduced-motion aparece pronta. O nível é **Select Tier Services** — não "Premier".

**Trabalhe conosco — mockup da equipe (24/09).** Duas faixas. **Preta:** "Trabalhe / conosco" (Syne 800, 74px; mínimo 40px no celular, porque "Trabalhe" a 48px não cabe em 312px), texto de `careers.intro` e botão Azul Solução "Acessar vagas →" para `/vagas`; à direita, o carrossel de fotos da equipe (`CareersCarousel`) sangrando até a borda da tela (utilitário `bleed-right`). O carrossel troca a cada 6s: a foto nova entra por cima com fade de 1,2s enquanto a anterior fica opaca embaixo, e a ativa desliza de 106% para 100%. Os indicadores são traços; o ativo é mais largo e se enche de laranja — a própria animação de preenchimento (`carousel-fill`) é o temporizador, então pausar é pausar a animação. Clicar num indicador (inclusive o ativo) reinicia o preenchimento do zero e a contagem segue correndo — por isso não há pausa por hover nem por foco, que deixavam a barra nova parada no zero. Pausa fora da tela e pelo botão de pausa (WCAG 2.2.2); com `prefers-reduced-motion` não avança sozinho. Fotos em `public/carreiras/equipe-01…05.webp` (da equipe, 1488px). **Branca:** "Como é trabalhar no SD" e quatro cartões laranja com texto preto (5,37:1), ícone Phosphor num quadrado claro: globo (remoto), foguete (alto impacto), bússola (autonomia), tendência de alta (remuneração e evolução); depois "Acompanhe nas redes:" com as três redes. As cinco áreas em texto da versão anterior saíram.
...
**Rodapé (Figma 188:21503 / 188:21611, reorganizado em 25/09).** Uma linha só: o símbolo do SD à esquerda (145px) e o mapa do site à direita, que é a navegação do rodapé (o "Navegar" em Syne 24px saiu — repetia o mapa). A coluna do logotipo reserva a largura dele aberto (24rem), então no hover ou foco "SENAI / SOLUÇÕES DIGITAIS" (`components/brand/Wordmarks.tsx`, exportadas do componente Logo do Figma) sai do símbolo sem mexer nas colunas (`grid-template-columns` 0fr→1fr em ease-out de 800ms, as palavras entram em sequência logo depois; na saída somem primeiro e a largura fecha em seguida). Abaixo de 1440px o logotipo fica em cima das colunas — ao lado elas ficavam estreitas demais. Mapa em quatro colunas (duas no celular): Serviços (as mesmas 5 da home → `/cases?capacidade=`), Cases (os 4 editoriais + todos), Empresa (Sobre nós, Parcerias, Parceria AWS, Seja nosso parceiro) + Carreiras (Trabalhe conosco, /vagas), Contato (`<address>` com e-mail, região e Fale conosco — sem o telefone fictício) + Redes (LinkedIn, Instagram, Facebook ↗); cada grupo com `h2` mono e lista rotulada. O rodapé publica o JSON-LD `Organization` (nome, url, logo, 2007, e-mail, região SC, redes). Hairline; "© ano. Todos os direitos reservados." e "Política de Privacidade" (texto — **PENDENTE**: não há página de política); e "SENAI SOLUÇÕES DIGITAIS" em Syne 800 caixa alta ocupando a largura (`100cqw / 22,5` — com `vw` a barra de rolagem entrava na conta). Essa base (linha, direitos, política e o nome) fica **fora do container de 1440**, em largura cheia com as margens laterais do site: em telas grandes a linha vai de margem a margem e o nome cresce junto; direitos sempre na ponta esquerda e política na direita, na mesma linha em qualquer tamanho. A navbar ganhou "Parcerias" (Cases, Serviços, Parcerias, Sobre, Trabalhe conosco).
...
**Taxonomia de capacidades (única)** — rotulada na UI como "Serviços"; a âncora é `#servicos` e os slugs continuam: 01 Plataformas e sistemas · 02 IA aplicada · 03 Apps móveis · 04 Dados e analytics · 05 VR/AR/simulação · 06 Cloud e DevOps na AWS (o detalhe da parceria vive em `partners.ts` — seção Parcerias e `/aws`).
...
| AWS como capacidade 06 **e** seção Parcerias + página `/aws` (25/09) | A parceria precisa de vitrine própria e de uma porta para novos parceiros | Só capacidade com redirect de `/aws` (versão anterior) |
...
- [ ] Confirmar "mais de 550 colaboradores" e "24 estados" (texto do mockup do Trabalhe conosco, sem outra fonte). O botão do mockup dizia "550 Vagas abertas"; publicado como "Acessar vagas" porque o número de vagas muda no Pandapé.

**Assets**
- [ ] Selo oficial AWS Partner em resolução maior (o PNG da equipe tem 313×270; fica um pouco mole em tela retina). Na seção Parcerias da home o selo ainda é texto.
- [ ] Validar o texto de "Quer construir junto?" (novas parcerias) — escrito para o site, sem material de origem.
...
**Feedback 207:744 (Figma, 25/09) — aplicado:** menu mobile sem números; botão "Enviar mensagem" com largura total e conteúdo centralizado no celular; sem globo no celular; "Vem ser FIESC" centralizado no celular; barra de progresso nos cases (já feita). Do WAVE: o número grande de cada case virou `h3` (com o nome do projeto só para leitor de tela; o logotipo tem `alt=""`); o resumo do case passou a 19px negrito (texto grande — o branco sobre o laranja, 3,9:1, passa AA); "Acompanhe nas redes:" virou `h3`; o "copiado" do e-mail saiu de 10px para 11px. O "texto alternativo redundante" apontado era do rodapé antigo, que não existe mais.
